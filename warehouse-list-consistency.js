/* PALA v187 · unified warehouse list rows
   Hardware items and category rows share one visual language.
   Only categories with child categories can collapse/expand. */
(()=>{
'use strict';
if(window.__palaWarehouseListConsistencyV187)return;
window.__palaWarehouseListConsistencyV187=true;
...
function enhanceCategory(details){
  ...
  const chevron=hasChildren&&typeof uiIcon==='function'?uiIcon('chevronRight'):'';
  summary.innerHTML=`<span class="warehouse-item-icon">${icon}</span><span class="warehouse-item-body"><strong>${escHtml(title)}</strong><small>${hasChildren?'Kategori med underkategorier':'Kategori'}</small><span class="warehouse-stock">${count} ${count===1?'post':'poster'}</span></span>${hasChildren?`<span class="warehouse-category-unified-chevron-v187">${chevron}</span>`:'<span class="warehouse-category-unified-spacer-v187" aria-hidden="true"></span>'}`;

  if(!hasChildren){
    details.open=true;
    summary.setAttribute('aria-disabled','true');
    summary.setAttribute('tabindex','-1');
    const keepOpen=()=>{if(!details.open)details.open=true};
    summary.addEventListener('click',event=>event.preventDefault());
    summary.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault()}});
...
if(!document.getElementById('pala-warehouse-list-consistency-v187-style')){
  const style=document.createElement('style');
  style.id='pala-warehouse-list-consistency-v187-style';
  style.textContent=`
    ...
    .warehouse-category-unified-v187>summary{list-style:none!important;display:flex!important;align-items:center!important;gap:12px!important;min-height:58px!important;padding:14px!important;background:#fff!important;border:0!important;border-bottom:1px solid #edf0f5!important;border-radius:0!important;font-weight:400!important}
    ...
    .warehouse-category-unified-chevron-v187,.warehouse-category-unified-spacer-v187{flex:0 0 24px!important;width:24px!important;display:flex!important;align-items:center!important;justify-content:center!important}
    .warehouse-category-unified-chevron-v187{transition:transform .16s ease!important;color:#405574!important}
    .warehouse-category-unified-v187[open]>summary>.warehouse-category-unified-chevron-v187{transform:rotate(90deg)!important}
    .warehouse-category-unified-v187.has-subcategories-v187>summary{cursor:pointer!important}
    .warehouse-category-unified-v187.leaf-category-v187>summary{cursor:default!important}
    .warehouse-category-unified-v187>.warehouse-category-body-v183{padding:0!important}
    .warehouse-category-unified-v187.leaf-category-v187>.warehouse-category-body-v183>.warehouse-list-v183{padding:0 0 0 18px!important;gap:0!important}
    ...
    @media(max-width:600px){
      ...
      .warehouse-category-unified-v187.leaf-category-v187>.warehouse-category-body-v183>.warehouse-list-v183{padding-left:12px!important}
