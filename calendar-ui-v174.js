...
(()=>{
'use strict';
if(window.__palaCalendarUiV311)return;
...
function enhanceCalendar(){
  try{
    ...
    summary.replaceChildren(
      iconButton('orders','calendar','Ordre','Ordrer',selectedTypes.includes('orders')),
      iconButton('staffing','people','Vagter','Vagter',selectedTypes.includes('staffing')),
      iconButton('workshop','scissors','Systue','Systue',selectedTypes.includes('workshop')),
      iconButton('calendarItems','check','Møder / opgaver','Møder og andre opgaver',selectedTypes.includes('calendarItems'))
    );
    ...
    const parts=[];
    if(selectedTypes.includes('orders'))parts.push(countNode(counts.orders));
    if(selectedTypes.includes('staffing')){
      const staffingParts=String(counts.staffing||'').split(/\s*·\s*/).filter(Boolean);
      parts.push(countNode(staffingParts[0]||'0 vagter'));
      if(counts.staffingMissing&&staffingParts.length>1)parts.push(countNode(staffingParts.slice(1).join(' · '),true));
    }
    if(selectedTypes.includes('workshop'))parts.push(countNode(counts.workshop));
    if(selectedTypes.includes('calendarItems'))parts.push(countNode(counts.calendarItems));
