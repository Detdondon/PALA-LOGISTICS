...
function eventTypeMatch(e){let t=mainCalendarTypeFilterV120,k=kind(e);if(t==='all')return true;if(t==='orders')return k==='order';if(t==='staffing')return k==='staffing'||k==='staffLeave';return k==='workshop'||k==='workshopTask'}
...
function dayHtml(ds){let type=mainCalendarTypeFilterV120,rows=[],add=(k,r,t='')=>rows.push({k,r,time:time(t),name:name(k,r),html:card(k,r)});if(type==='all'||type==='orders')(bookingsForCalendarDate(ds)||[]).forEach(r=>{if(rowMatch('order',r))add('order',r,r.start_time)});if(type==='all'||type==='staffing')staffingShifts.forEach(r=>{let on=leave(r)?staffLeaveContains(r,ds):date(r.shift_date)===ds;if(on&&rowMatch('staffing',r))add('staffing',r,r.start_time)});if(type==='all'||type==='workshop'){workshopJobs.forEach(r=>{if(r.start_date<=ds&&r.end_date>=ds&&rowMatch('workshop',r))add('workshop',r,r.start_time)});workshopTasks.forEach(r=>{if(!rowMatch('workshopTask',r)||typeof workshopTaskRangeV120!=='function')return;let x=workshopTaskRangeV120(r);if(x&&x.start<=ds&&x.end>=ds)add('workshopTask',r)})}if(type==='all'&&['all','active'].includes(mainCalendarStatusFilterV123))(palaMeetings||[]).forEach(r=>{if(!r.cancelled&&r.start_date<=ds&&r.end_date>=ds)add('meeting',r,r.start_time)});let p={order:1,staffing:2,workshop:3,workshopTask:4,meeting:5};rows.sort((a,b)=>a.time.localeCompare(b.time)||(p[a.k]||9)-(p[b.k]||9)||cmp(a.name,b.name)||(+a.r?.id||0)-(+b.r?.id||0));return rows.map(r=>r.html).join('')||'<p class="calendar-empty">Ingen aktiviteter matcher filteret denne dag.</p>'}mainCalendarDayHtmlV120=dayHtml;
