...
const TYPES=['all','orders','staffing','workshop','calendarItems'],COMMON=['all','active','completed'],SORTS=['date_asc','date_desc','name_asc','name_desc','type','status'],legacyEvents=calendarEvents,baseOrderCard=orderCard,locale='da-DK';
...
function allowed(type){if(type==='calendarItems')return['all','active'];let x=[...COMMON];if(type==='orders')x.push('inquiry','warehouse','out');if(type==='staffing')x.push('understaffed');return x}
...
mainCalendarTypeFilterV120=normType(mainCalendarTypeFilterV120);mainCalendarStatusFilterV123=normStatus(mainCalendarStatusFilterV123,mainCalendarTypeFilterV120);calendarViewMode='calendar';localStorage.setItem('pala_calendar_view','calendar');
...
function eventTypeMatch(e){let t=mainCalendarTypeFilterV120,k=kind(e);if(t==='all')return true;if(t==='orders')return k==='order';if(t==='staffing')return k==='staffing'||k==='staffLeave';if(t==='workshop')return k==='workshop'||k==='workshopTask';if(t==='calendarItems')return k==='meeting'||k==='task';return false}
const eventName=e=>e?.name||e?.item?.customer_name||e?.item?.title||e?.item?.tent_name||e?.item?.description||'',eventKey=e=>e?.key||`${kind(e)}-${e?.item?.id||e?.id||eventName(e)}-${e?.start||''}-${e?.end||''}`,priority=e=>({order:1,staffing:2,staffLeave:2,workshop:3,workshopTask:4,meeting:5,task:5})[kind(e)]||9;
...
function statusRows(type){if(type==='calendarItems')return[['all','Alle'],['active','Aktive']];let r=[['all','Alle'],['active',type==='workshop'?'Åbne':'Aktive'],['completed','Afsluttede']];if(type==='orders')r.push(['inquiry','Forespørgsler'],['warehouse','På lager'],['out','Ude']);if(type==='staffing')r.push(['understaffed','Ubemandede vagter']);return r}
...
const time=v=>/^\d\d:\d\d/.test(text(v))?text(v).slice(0,5):'',card=(k,r)=>k==='order'?orderCard(r):k==='staffing'?staffShiftCard(r):k==='workshop'?workshopJobCard(r):k==='workshopTask'?(typeof calendarWorkshopTaskCardV310==='function'?calendarWorkshopTaskCardV310(r):workshopTaskCard(r)):meetingCard(r);
function entries(){let w=monthWindow(),type=mainCalendarTypeFilterV120,out=[],add=(k,r,start,t='')=>out.push({k,r,date:max(date(start),w.first),time:time(t),name:name(k,r),status:statusLabel(k,r),html:card(k,r)});if(type==='all'||type==='orders')bookings.forEach(r=>{let a=date(r?.start_date),b=date(r?.end_date);if(a&&b&&a<=w.last&&b>=w.first&&rowMatch('order',r))add('order',r,a,r.start_time)});if(type==='all'||type==='staffing')staffingShifts.forEach(r=>{if(!rowMatch('staffing',r))return;let l=typeof staffLeaveInfo==='function'?staffLeaveInfo(r):null;if(l){if(l.start<=w.last&&l.end>=w.first)add('staffing',r,l.start)}else{let d=date(r.shift_date);if(d>=w.first&&d<=w.last)add('staffing',r,d,r.start_time)}});if(type==='all'||type==='workshop'){workshopJobs.forEach(r=>{let a=date(r?.start_date),b=date(r?.end_date);if(a&&b&&a<=w.last&&b>=w.first&&rowMatch('workshop',r))add('workshop',r,a,r.start_time)});workshopTasks.forEach(r=>{if(type!=='workshop'||!rowMatch('workshopTask',r)||typeof workshopTaskRangeV120!=='function')return;let x=workshopTaskRangeV120(r);if(x&&x.start<=w.last&&x.end>=w.first)add('workshopTask',r,x.start)})}if((type==='all'||type==='calendarItems')&&['all','active'].includes(mainCalendarStatusFilterV123))(palaMeetings||[]).forEach(r=>{let a=date(r?.start_date),b=date(r?.end_date),k=r?.kind==='task'?'task':'meeting';if(!r?.cancelled&&a&&b&&a<=w.last&&b>=w.first)add(k,r,a,r.start_time)});return out}
...
/* PALA v297 · the list below the calendar starts at the selected day and continues forward. */
function selectedDateForwardEntriesV297(){
  let first=staffDateString(calDate),type=mainCalendarTypeFilterV120,out=[];
  let add=(k,r,start,end,t='')=>{
    start=date(start);end=date(end||start);
    if(!start||!end||end<first)return;
    out.push({k,r,date:max(start,first),time:time(t),name:name(k,r),status:statusLabel(k,r),html:card(k,r)});
  };
  if(type==='all'||type==='orders')bookings.forEach(r=>{
    let a=date(r?.start_date),b=date(r?.end_date);
    if(a&&b&&b>=first&&rowMatch('order',r))add('order',r,a,b,r.start_time);
  });
  if(type==='all'||type==='staffing')staffingShifts.forEach(r=>{
    if(!rowMatch('staffing',r))return;
    let l=typeof staffLeaveInfo==='function'?staffLeaveInfo(r):null;
    if(l){if(l.end>=first)add('staffing',r,l.start,l.end)}
    else{let d=date(r.shift_date);if(d>=first)add('staffing',r,d,d,r.start_time)}
  });
  if(type==='all'||type==='workshop'){
    workshopJobs.forEach(r=>{
      let a=date(r?.start_date),b=date(r?.end_date);
      if(a&&b&&b>=first&&rowMatch('workshop',r))add('workshop',r,a,b,r.start_time);
    });
    if(type==='workshop')workshopTasks.forEach(r=>{
      if(!rowMatch('workshopTask',r)||typeof workshopTaskRangeV120!=='function')return;
      let x=workshopTaskRangeV120(r);
      if(x&&x.end>=first)add('workshopTask',r,x.start,x.end);
    });
  }
  if((type==='all'||type==='calendarItems')&&['all','active'].includes(mainCalendarStatusFilterV123))(palaMeetings||[]).forEach(r=>{
    let a=date(r?.start_date),b=date(r?.end_date),k=r?.kind==='task'?'task':'meeting';
    if(!r?.cancelled&&a&&b&&b>=first)add(k,r,a,b,r.start_time);
  });
  return out;
}
function monthDetailHtml(){
  let rows=selectedDateForwardEntriesV297().sort((a,b)=>a.date.localeCompare(b.date)||a.time.localeCompare(b.time)||cmp(a.name,b.name)||(+a.r?.id||0)-(+b.r?.id||0));
  if(!rows.length)return '<p class="calendar-empty">Ingen aktiviteter matcher filtrene fra den valgte dato og frem.</p>';
  let split=splitPinnedWorkshopRows(rows);
  return renderPinnedWorkshopRows(split.pinned)+renderRowsGroupedByDate(split.rest);
}
window.calendarMonthDetailHtmlV157=monthDetailHtml;
function dayHtml(ds){let type=mainCalendarTypeFilterV120,rows=[],add=(k,r,t='')=>rows.push({k,r,time:time(t),name:name(k,r),html:card(k,r)});if(type==='all'||type==='orders')(bookingsForCalendarDate(ds)||[]).forEach(r=>{if(rowMatch('order',r))add('order',r,r.start_time)});if(type==='all'||type==='staffing')staffingShifts.forEach(r=>{let on=leave(r)?staffLeaveContains(r,ds):date(r.shift_date)===ds;if(on&&rowMatch('staffing',r))add('staffing',r,r.start_time)});if(type==='all'||type==='workshop'){workshopJobs.forEach(r=>{if(r.start_date<=ds&&r.end_date>=ds&&rowMatch('workshop',r))add('workshop',r,r.start_time)});workshopTasks.forEach(r=>{if(type!=='workshop'||!rowMatch('workshopTask',r)||typeof workshopTaskRangeV120!=='function')return;let x=workshopTaskRangeV120(r);if(x&&x.start<=ds&&x.end>=ds)add('workshopTask',r)})}if((type==='all'||type==='calendarItems')&&['all','active'].includes(mainCalendarStatusFilterV123))(palaMeetings||[]).forEach(r=>{if(!r.cancelled&&r.start_date<=ds&&r.end_date>=ds)add(r.kind==='task'?'task':'meeting',r,r.start_time)});let p={order:1,staffing:2,workshop:3,workshopTask:4,meeting:5,task:5};rows.sort((a,b)=>a.time.localeCompare(b.time)||(p[a.k]||9)-(p[b.k]||9)||cmp(a.name,b.name)||(+a.r?.id||0)-(+b.r?.id||0));return rows.map(r=>r.html).join('')||'<p class="calendar-empty">Ingen aktiviteter matcher filteret denne dag.</p>'}mainCalendarDayHtmlV120=dayHtml;
